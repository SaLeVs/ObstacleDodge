...
float3 Glitch(float3 vertexPos, float3 shaderTime)
{
	...
	float3 glitchDir = mul((float3x3)unity_WorldToObject, ACCESS_PROP_FLOAT3(_GlitchOffset));
	float3 scale = float3(length(unity_ObjectToWorld[0].xyz),
					length(unity_ObjectToWorld[1].xyz),
					length(unity_ObjectToWorld[2].xyz));
	float pos = ACCESS_PROP_FLOAT(_GlitchWorldSpace) ? mul(unity_ObjectToWorld, float4(vertexPos, 1.0)).y : vertexPos.y;
